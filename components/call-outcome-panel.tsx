...
import { PhoneOff, Voicemail, UserCheck, UserX, Clock, UserPlus, Search, X, Building2 } from "lucide-react"
...
// Shared post-call disposition UI: outcome picker, notes, and contact linking.
// Used after any call ends (Quick Dial, the prospect call dialog, inbound calls)
// so logging a call looks and behaves identically everywhere.
export function CallOutcomePanel({
  ...
  phoneNumber: string
  duration: number
  // If the call is already attributed to a prospect (e.g. a matched inbound
  // caller), skip the new/existing/skip picker and just show who it was.
  knownProspect?: { id: string; name: string } | null
  // Notes taken live during the call, carried over so they aren't lost
  initialNotes?: string
  onSaved: () => void
  onDiscard: () => void
}) {
  ...
  const saveCall = async () => {
    ...
    setSaving(true)
    try {
      let prospectId: string | null = knownProspect?.id || null

      if (!knownProspect && contactAction === "new" && newName.trim()) {
      ...
      } else if (!knownProspect && contactAction === "existing" && selectedProspect) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/40 border border-border">
        <span className="text-sm font-medium">{knownProspect?.name || phoneNumber}</span>
        <span className="text-xs text-muted-foreground">Duration: {formatDuration(duration)}</span>
      </div>
      ...
      {knownProspect ? (
        <p className="text-xs text-muted-foreground">
          Linked to <span className="font-medium text-foreground">{knownProspect.name}</span>
        </p>
      ) : (
        <div className="space-y-3">
